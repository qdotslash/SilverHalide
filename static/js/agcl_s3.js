var xenonBucket = 'mendeleev-xenon-prod';

function ajax_get_creds(){
...
async function da_creds(){
    let creds = await ajax_get_creds();
    return creds
}
var creds = da_creds()
...
var s3 = new AWS.S3({
  ...
  params: {Bucket: xenonBucket}
});

function list_images(path) {
    var pathKey = encodeURIComponent(path) + '/';
    console.log('Path key: ' + pathKey)
    s3.listObjects({Prefix: pathKey}, function(err, data) {
      if (err) {
        return alert('There was an error viewing your album: ' + err.message);
      }
      // 'this' references the AWS.Response instance that represents the response
      var href = this.request.httpRequest.endpoint.href;
      var bucketUrl = href + xenonBucket + '/';
      return data.Contents.map(funcion(photo))
    });
  }
